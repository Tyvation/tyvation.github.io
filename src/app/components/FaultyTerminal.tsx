...
float noise(vec2 p)
{
  ...
  float base = sin(p.x * 4.0 + time) * sin(p.y * 5.0 + time * 0.7);
  float detail = sin(p.x * 12.0) * sin(p.y * 12.0) * 0.3;
  return base * 0.7 + detail;
}
...
float fbm(vec2 p)
{
  // Simplified to single octave for performance
...
vec3 getColor(vec2 p){
    ...
    // Simplified sampling - reduce from 9 to 1 sample for performance
    vec3 onColor = vec3(0.9) * middle * bar;
    vec3 baseColor = mix(uBackgroundColor, onColor, step(0.01, middle));
    return baseColor;
}
...
export default function FaultyTerminal({
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    // Use global mouse position relative to viewport
  ...
  useEffect(() => {
    ...
    const finalDpr = dpr || Math.min(window.devicePixelRatio || 1, 2);
